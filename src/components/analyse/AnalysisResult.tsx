    ...
    <div className="flex flex-col gap-4">
      ...
      {result.currentPrice && (
        ...
        >
          ...
          <span className="text-xs" style={{ color: 'var(--text-3)' }}>Aktueller Kurs (Echtzeit)</span>
      ...
      >
        ...
        <div>
          <p className="text-xs font-medium uppercase tracking-wider" style={{ color: bias.color, opacity: 0.8 }}>Markt-Bias</p>
          <p className="text-2xl font-bold" style={{ color: bias.color }}>{bias.label}</p>
        </div>
        <div className="ml-auto flex items-center gap-1.5">
          <span className="text-xs" style={{ color: 'var(--text-3)' }}>Konfidenz</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard icon={Crosshair} label="Entry-Zone" value={result.entry_zone} delay={0.05} />
        <MetricCard icon={ShieldAlert} label="Stop Loss" value={result.stop_loss} valueColor="#ef4444" delay={0.1} />
        <MetricCard icon={Target} label="Take Profit" value={result.take_profit} valueColor="#22c55e" delay={0.15} />
        <MetricCard icon={BarChart2} label="Risk/Reward" value={result.risk_reward} delay={0.2} />
      </div>
