"""
Lokales Log fuer den AI-Trading Bot.
Speichert Eintraege in bot_log.json und pusht sie an AlphaTrack.
Schreibt ausschliesslich bot-relevante Ereignisse — keine Bridge-internen Logs.
"""
...
MAX_ENTRIES = 2000
_LOG_FILE = os.path.join(os.path.dirname(__file__), "bot_log.json")
...
class BotLog:
    """Bot-spezifisches Log. Schreibt nur bot-relevante Ereignisse."""
    ...
    def __init__(self, bot_id: str, bot_name: str):
        ...
        self._worker = threading.Thread(target=self._push_worker, daemon=True, name="BotLogPush")
        self._worker.start()

    def configure_push(self, alphatrack_url: str, api_key: str) -> None:
        """Konfiguriert den Live-Push neuer Eintraege zu AlphaTrack."""
    ...
    def _push_entry(self, entry: dict) -> None:
        try:
            _requests.post(
                ...
                json={"botId": self._bot_id, "entries": [entry]},
                headers={
